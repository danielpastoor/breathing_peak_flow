...
data_file <- "raw_data/Data_breath.csv" #"../raw_data/Data_breath.csv"
...
```
Hieruit kan worden gehaald dat alle mogelijke combinaties normaal verdeeld zijn.

De datasets zijn normaal verdeeld, er wordt gekeken of er verschil zit tussen de verschillende sexsen.

```{r verschil tussen de verschillende sexsen}
t.test(data_woman$Peak_flow, y = data_man$Peak_flow)
```
De p-waard is 6.102e-11 dit betekent dat er een verschil aanwezig is tussen de peakflow van de mannen en vrouwen.

Zit er een verschil tussen sporten en niet sportende mensen?
```{r verschil tussen sportende en niet sportende}
data_breath %>%
  group_by(Sex, Sporting)%>%
  pivot_wider(names_from = Sporting, values_from = Peak_flow) %>%
  summarise(t.test(TRUE, y = FALSE, alternative = "greater" ))

```



Hiernaast wordt er gekeken of er meer verschillen zit bij de verschillende groepen voor en na 10 jumping jacks.

```{r verschillende t.test op groepen}
data_breath %>%
  group_by(Sex, Has_activity, Sporting) %>%
...
Is er een effect van de factoren: sportende, uitvoering van kleine sportieve activiteit en geslacht op de gemeten peak flow. Dit wordt gedaan doormiddel van een anova test.
