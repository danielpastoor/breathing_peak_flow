...
```{r label = 'normaal verdeling van data', echo=FALSE, fig.cap='Per verschillende groep de p-value bij een Shapiro-Wilk test}
data_breath %>% 
...
```{r label = verschil tussen de verschillende sexsen, echo=FALSE, fig.cap='T-test of er een verschil in peakflow zit bij mannen en bij vrouwen'}
...
```{r label = verschillende t.test op groepen, echo=FALSE, fig.cap='De verschillende p-values van de t-testen bij de verschillende groepen voor en na 10 jumping'}
data_breath %>%
  group_by(Sex, Has_activity, Sporting) %>%
...
```{r label = ANOVA, echo=FALSE, fig.cap='P values voor ANOVA testen van de effecten van sporten, een sportieve activiteit en het geslacht. }
pvalue <- c()
